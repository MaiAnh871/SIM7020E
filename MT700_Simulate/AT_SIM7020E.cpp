...
void AT_SIM7020E::setupModule(String address, String port){
  ...
  Serial.println(F("\n>>SET RECEIVE & SENDING FLAG WITH STRING"));
  setReceiveFlag("string");
  setTCPSendFlag(1);
  _serial_flush();
...
void AT_SIM7020E::eDRXSetting(){
  _Serial->println(F("AT+CEDRXS=2,5,\"1111\""));
  while (1){
    if (_Serial->available()){
      dataInput = _Serial->readStringUntil('\n');
      if (dataInput.indexOf(F("OK")) != -1){
...
void AT_SIM7020E::powerSavingMode(unsigned int psm){
  ...
  _Serial->println(psm);
  _serial_flush();
  Serial.println(F("Set Power Saving Mode successfully"));
}

// Power Saving Mode Setting
void AT_SIM7020E::psmSetting(){
  _Serial->println(F("AT+CPSMS=1,,,\"00111001\",\"11100000\""));
  while (1){
    if (_Serial->available()){
      dataInput = _Serial->readStringUntil('\n');
      if (dataInput.indexOf(F("OK")) != -1){
...
void AT_SIM7020E::sendData(String address, String port, unsigned int len, String payload){
  ...
  while (1){
    if (_Serial->available()){
      Serial.println("ALL: Co du lieu tra ve");
      dataInput = _Serial->readString();
      Serial.println(dataInput);
      break;
    }
    else{
      Serial.println("ALL: Khong co du lieu tra ve");
      break;
    }
  }
...
void AT_SIM7020E::_Serial_print(String address, String port, unsigned int len){
  ...
  while (1){
    if (_Serial->available()){
      ...
      Serial.print("ALL: Khong co du lieu tra ve");
      break;
    }
  }
}
...
void AT_SIM7020E::_Serial_print(String msg){
  ...
  while (1){
    if (_Serial->available()){
      Serial.println("STRING: Co du lieu tra ve");
      dataInput = _Serial->readString();
      Serial.println(dataInput);
      break;
    }
    else{
      Serial.print("STRING: Khong co du lieu tra ve");
      break;
    }
  }
}
...
void AT_SIM7020E::_Serial_print(unsigned int msg){
  ...
  while (1){
    if (_Serial->available()){
      Serial.println("INT: Co du lieu tra ve");
      dataInput = _Serial->readString();
      Serial.println(dataInput);
      break;
    }
    else{
      Serial.print("INT: Khong co du lieu tra ve");
      break;
    }
  }
}
...
void AT_SIM7020E::_Serial_print(char *msg){
  ...
  while (1){
    if (_Serial->available()){
      Serial.println("CHAR: Co du lieu tra ve");
      dataInput = _Serial->readString();
      Serial.println(dataInput);
      break;
    }
    else{
      Serial.print("CHAR: Khong co du lieu tra ve");
      break;
    }
  }
}
...
void AT_SIM7020E::_Serial_println(){
  ...
  while (1){
    if (_Serial->available()){
      Serial.println("RN: Co du lieu tra ve");
      dataInput = _Serial->readString();
      Serial.println(dataInput);
      break;
    }
    else{
      Serial.print("RN: Khong co du lieu tra ve");
      break;
    }
  }
}
